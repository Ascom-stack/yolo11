...
from ultralytics.nn.extra_modules.block import RepNCSPELAN4, C3k2_Faster_CGLU, C3k_Faster_CGLU, \
    C3k2_MutilScaleEdgeInformationEnhance, C3k_MutilScaleEdgeInformationEnhance, C3k2_Faster_EMA, C3k_Faster_EMA, \
    Faster_Block_EMA
...
def replace_c2f_with_c2f_v2(module):
    ...
    # for yolo11-C3k2-Faster-CGLU-RSCD.yaml
    for name, child_module in module.named_children():
        if isinstance(child_module, C3k2_Faster_EMA):
            # Replace C2f with C2f_v2 while preserving its parameters
            shortcut = infer_shortcut(child_module.m[0])
            c3k = child_module.m[0] is C3k_Faster_EMA
            if c3k:
                g = 1
            else:
                g = 1
            c2f_v2 = C3k2_Faster_EMA_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
                            n=len(child_module.m), shortcut=shortcut,
                            g=g,
                            c3k=c3k,
                            e=child_module.c / child_module.cv2.conv.out_channels)
