...
from ultralytics.nn.extra_modules.block import Faster_Block, Fusion, IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, \
    ...
    CSPOmniKernel, SPDConv, Faster_Block_EMA, C3k2_Faster_EMA, C3k_Faster_EMA
...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, PSABlock):
            ignored_layers.append(m.attn)
        if isinstance(m, Faster_Block_EMA):
            ignored_layers.append(m.mlp[-1])  # 最后的转换层
            ignored_layers.append(m.attention)  # 最后的转换层
             # EMA注意力层# 完全忽略整个Faster_Block_EMA模块
        # 最后的转换层
        if isinstance(m, RepVGGBlock):
            ignored_layers.append(m)
    ...
    # for yolo11-C3k2-Faster-EMA-RSCD.yaml
    # for k, m in model.named_modules():
    #
    #     if isinstance(m, PSABlock):
    #         ignored_layers.append(m.attn)
